...
func (s *interposeDataRegistryServer) Register(ctx context.Context, nse *registry.NetworkServiceEndpoint) (*registry.NetworkServiceEndpoint, error) {
	logrus.Infof("EZOLLUG: interposeDataRegistryServer: Register: nseName=%v, nse=%v", nse.Name, nse)
	r, err := next.NetworkServiceEndpointRegistryServer(ctx).Register(ctx, nse)
	if err != nil {
		return nil, err
	}

	logrus.Infof("EZOLLUG: interposeDataRegistryServer: Register: result nseName=%v, nse=%v", r.Name, r)
	if interpose.Is(nse.Name) {
		logrus.Infof("EZOLLUG: interposeDataRegistryServer: Register: storing nse=%v", r)
...
func (s *interposeDataRegistryServer) Find(query *registry.NetworkServiceEndpointQuery, server registry.NetworkServiceEndpointRegistry_FindServer) error {
	logrus.Infof("EZOLLUG: interposeDataRegistryServer: Find: query=%v, server=%v", query, server)
...
func (s *interposeDataRegistryServer) Unregister(ctx context.Context, nse *registry.NetworkServiceEndpoint) (*empty.Empty, error) {
	logrus.Infof("EZOLLUG: interposeDataRegistryServer: Unregister: nse=%v", nse)
	if interpose.Is(nse.Name) {
		logrus.Infof("EZOLLUG: interposeDataRegistryServer: Unregister: remove nse.Name=%v", nse.Name)
