...
func (l *interposeCandidateServer) Request(ctx context.Context, request *networkservice.NetworkServiceRequest) (result *networkservice.Connection, err error) {
	...
	l.interposeData.Range(func(theKey string, crossNSE *registry.NetworkServiceEndpoint) bool {
		if ok := matchEndpoint(request.GetConnection().GetLabels(), crossNSE); ok {
			logrus.Infof("interposeCandidateServer: Request: MATCH!!! theKey=%v crossNSE=%v reqLabels=%v", theKey, crossNSE, request.GetConnection().GetLabels())
		...
		logrus.Infof("interposeCandidateServer: Request: No match! theKey=%v crossNSE=%v reqLabels=%v", theKey, crossNSE, request.GetConnection().GetLabels())
