	...
	if ok && (fromPath || int(conn.Path.Index) < len(conn.Path.PathSegments)-1) {
		...
		if candidateCrossNSEName, ok := interposecandidate.Candidate(ctx); ok {
			...
			if candidateCrossNSEURL, ok = l.interposeNSEs.Load(candidateCrossNSEName); ok {
				logrus.Infof("interposeServer: Request: candidateCrossNSEName=%v, candidateCrossNSEURL=%v", candidateCrossNSEName, candidateCrossNSEURL)
			...
			l.interposeNSEs.Range(func(theKey string, crossNSEURL *url.URL) bool {
				logrus.Infof("interposeServer: Request: theKey=%v, crossNSEURL=%v", theKey, crossNSEURL)
